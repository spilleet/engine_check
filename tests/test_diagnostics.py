...
def test_diagnose_and_recommend():
    ...
    그리고 정비 요청 및 승인서 마크다운 생성(Report) 기능을 통합 검증하는 유닛 테스트.
    """
    ...
    # 4. MaintenanceReport 에이전트의 마크다운 정비 요청 및 승인서 자동 생성 포맷 검증
    ...
    # 생성된 문서 마크다운 내에 장비 번호, 정비 필요 장비 부품명, 관제사 코멘트가 포함되어 있는지 검증
    ...
    res_inspect = alert_agent.run_alert_logic(unit_id=3, tick=720, status="inspect", rul=45.0, anomalies=["s_11"])
    assert "대시보드 로그 전용" in res_inspect
    
    # 케이스 B: 주간 근무 시간대 (12:00 -> 틱 720)이고 danger(위험) 상태 발생 -> 슬랙 알림 발신
    res_day_danger = alert_agent.run_alert_logic(unit_id=3, tick=720, status="danger", rul=18.0, anomalies=["s_11"])
    ...
    res_night_safe = alert_agent.run_alert_logic(unit_id=3, tick=120, status="danger", rul=18.0, anomalies=["s_11"])
    ...
    res_night_urgent = alert_agent.run_alert_logic(unit_id=9, tick=120, status="danger", rul=15.0, anomalies=["s_11", "s_12"])
